	...
	<div className=" w-full  flex flex-col items-center border-b-2 border-gray-500 h-full  background-img pb-10">	
		<div className=" w-full min-h-screen md:hidden ">
			<ul className="h-full w-full flex flex-col items-center">
			  {posts.map((post) => (
				<li className="bg-purple-100 pl-5 pb-5 pt-5 m-5 border-2" key={post.name}>
				  <Link href={`/content/blog/post/?id=${post.name}`} className="w-full">
					<Image
		...
		<div className="w-full min-h-screen hidden md:grid grid-cols-5">
			<ul className="col-start-2 col-span-3">
			  {posts.map((post) => (
				<li className="grid grid-cols-4 gap-4 pl-5 bg-purple-100 pb-5 pt-5 mt-5 mb-5 border-5 " key={post.name}>
				  <Link href={`/content/blog/post?id=${post.name}`} className="flex w-full col-span-4">
